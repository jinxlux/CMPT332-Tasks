...
struct proc *next_proc() {
    ...
    if(ListCount(queue1) != 0){
        p = ListTrim(queue1);
        return p;
    }
    if(ListCount(queue2) != 0){
        p = ListTrim(queue2);
        return p;
    }
    if(ListCount(queue3) != 0){
        p = ListTrim(queue3);
        return p;
    }
    if(ListCount(queue4) != 0){
        p = ListTrim(queue4);
        return p;
    }
    if(ListCount(queue5) != 0){
        p = ListTrim(queue5);
        return p;
    }
    return NULL;
...
void set_state(enum pstate state) {
    ...
    for (p = ptable.procs; p < &ptable.procs[ptable.size]; p++) {
        ...
        if (state == RUNNABLE) {
            ...
            if(p -> priority == 0){
                ListPrepend(queue1,p);
            }
            else if(p -> priority == 1){
                ListPrepend(queue2,p);
            }
            else if(p -> priority == 2){
                ListPrepend(queue3,p);
            }
            else if(p -> priority == 3){
                ListPrepend(queue4,p);
            }
            else{
                ListPrepend(queue5,p);
            }
        }
