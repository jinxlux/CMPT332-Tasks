...
DWORD WINAPI ThreadFunction(void * threadInfo){
    ...
    timeElapsed = (double)(endTime.wMinute - beginTime.wMinute) * 60 + (double)(endTime.wSecond - beginTime.wSecond) + (double)(endTime.wMilliseconds - beginTime.wMilliseconds) * 0.001;
    ...
    if(checkNumber(argv[1]) && checkNumber(argv[2]) &&checkNumber(argv[3])){
        ...
        */
        ...
        for(i = 0; i < threadNumber; i++){
            ...
            /*store size number */
            thread_Information[i] -> size_Number = sizeNumber;
