...
void execMyself(char* command, char **parameter){
	char *pathEnviroment;
    int checkExceRunning;
	char *commandPath;
    int i;
    /* declare the loacation array of command */
	char *commandLocation[5];
	commandPath = (char*)malloc(sizeof(char)*MAX_PATH_LEN);    
    checkExceRunning = -1;
	/* give five correct locations to find the correct command file */
    commandLocation[0] = "/bin/";
    commandLocation[1] = "/sbin/";
    commandLocation[2] = "/usr/bin/";
    commandLocation[3] = "/usr/sbin/";
    commandLocation[4] = "/usr/local/bin/";
	pathEnviroment = getenv("PATH");
	if(NULL == pathEnviroment){
		printf("couldn't get path variable\n");
	}
	else{
        /* use execvp to test other part */
		/*if(execvp(command, parameter) == -1){
			printf("wrong input command\n");
		}*/
        
        /* copy command to command location and use execv function*/
        for(i = 0; (i < 5) && (checkExceRunning == -1); i++){
            strcpy(commandPath,commandLocation[i]);
			strcat(commandPath,command);
        	if(execv(commandPath, parameter) == -1){
        	    checkExceRunning = -1;
        	}
			else{
				checkExceRunning = 1;
...
void commandPrompt(char *promptInfo){
	...
	*/
	...
	if(gethostname(hostName, MAX_NAME_LEN) == 0){
		sprintf(promptInfo,"[cmptShell]%s@%s:", userName, hostName);
	}
	else{
		sprintf(promptInfo,"[cmptShell]%s@unknown:", userName);
	}
	/*printf("%s\n", promptInfo);*/
